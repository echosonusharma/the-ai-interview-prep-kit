...
void main() {
  ...
  gl_FragColor = vec4(clamp(col, 0.0, 1.0), 1.0);
    ...
    // Fullscreen triangle: 3 vertices covering clip space
    ...
    colors.forEach((c, i) => {
      const loc_ = uColorLocs[i];
      if (loc_) gl.uniform3f(loc_, c[0], c[1], c[2]);
    });

    gl.uniform4f(uShape, 2.0, 0.54, 0.47, 0.04);
    gl.uniform4f(uSurface, 1.54, 1.16, 0.0, 1.0);
    gl.uniform4f(uFinish, 0.0, 0.21, 0.002, 0.1);
    gl.uniform4f(uTransform, 4012.0, 5.65, 0.12, 0.0);
    gl.uniform4f(uSpace, 0.11, -0.19, 0, 0);
    // cursor off
    gl.uniform4f(uCursor, 0, 2.0, 0.65, 0.46);

    // Resize with DPR cap 2
    const resize = () => {
    ...
    const onVis = () => {
      ...
      if (!paused) {
        // reset start offset to avoid jump
    ...
    const draw = (now: number) => {
      if (paused) return;
      const seconds = prefersReduced ? 0 : (now - start) / 1000;
      const t = seconds * -0.73;
      // u_scene = vec4(width, height, time, colorCount)
  ...
  return (
    <div className="absolute inset-0 -z-10 overflow-hidden bg-[#031C26]">
