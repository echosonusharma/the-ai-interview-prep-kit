...
The job description is pasted directly into the interface as text, not fetched from a job board. Most boards block automated access, and we would rather you spent your time on the interesting part.
...
- Your research and generation steps, and the second pass closing a coverage gap
- Editing and reordering, and a regeneration that preserves your edits
- Practice mode and the schedule
- Your custom feature, if you added one, and one design decision you would defend
...
- `priority` is `must | nice`
- `category` is `technical | behavioural | system-design | company-fit`
- `difficulty` is `1` to `3`
- `minutes` is an integer
- Every `id` is stable within a kit, and every `question_ids` entry in the schedule must refer to a question that exists.
